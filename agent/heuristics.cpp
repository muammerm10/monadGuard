...
HEURISTICS_RESULT runHeuristics(const vector<uint8_t> &data,
                                const string &fileHash, bool apiMode) {
  ...
  if (!apiMode) cout << "    Entropy: " << fixed << setprecision(2) << entropy << endl;
  
  if (entropy > 7.2) {
    if (!apiMode) cout << "    [!] ALERT: High Entropy / Potential Packer detected!" << endl;
    score += 30;
  }
  ...
  if (peRes.isPE) {
    if (!apiMode) cout << "[+] Windows PE File Detected." << endl;
    ...
    if (elfRes.isELF) {
      if (!apiMode) cout << "[+] Linux ELF File Detected." << endl;
      score += elfRes.scoreModifier;
      impHash = elfRes.impHash;
    } else {
      if (!apiMode) {
        cout << "[-] Unknown file format. Cannot perform deep structural analysis." << endl;
      }
    }
  }
  ...
  if (!impHash.empty()) {
    if (!apiMode) cout << "[+] Checking Anti-Farming Database for ImpHash..." << endl;
    if (find(KNOWN_MALICIOUS_IMPHASHES.begin(), KNOWN_MALICIOUS_IMPHASHES.end(),
             impHash) != KNOWN_MALICIOUS_IMPHASHES.end()) {
      if (!apiMode) {
        cout << "    [!] ALERT: Cheap Clone Detected! Different File Hash but identical ImpHash." << endl;
        cout << "    Reward Farming Blocked." << endl;
      }
      res.isCheapClone = true;
    } else {
      if (!apiMode) cout << "    [+] ImpHash clean. Proceeding." << endl;
    }
  }
